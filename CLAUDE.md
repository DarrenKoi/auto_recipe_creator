...
- **Zoom in/out ladder probe** (`monitor/cycle.py:_run_zoom_ladder`; check-only only; when feasibility verdict is `ambiguous`/`not_visible` — can't tell which point is the align key — sweep the live SEM box with the mouse wheel both OUT (lower mag, wide context) and IN (higher mag, pin the point), capturing each rung): `ALIGN_FAIL_ZOOM_PROBE` (0, opt-in), `ALIGN_FAIL_ZOOM_PROBE_STEPS` (2, OUT steps), `ALIGN_FAIL_ZOOM_PROBE_IN_STEPS` (2, IN steps), `ALIGN_FAIL_ZOOM_PROBE_SCROLL_DY` (-1, negative = wheel down = OUT/lower mag; IN flips sign), `ALIGN_FAIL_ZOOM_PROBE_SCROLLS_PER_STEP` (1, bump if one notch ≠ one PM step), `ALIGN_FAIL_ZOOM_PROBE_SETTLE_SEC` (0.6, post-wheel redraw), `ALIGN_FAIL_ZOOM_AIM_SETTLE_SEC` (0.15, hover settle after moving cursor into the box, before the wheel — module const in cycle.py), `ALIGN_FAIL_ZOOM_PROBE_REMATCH` (1; re-match the rcp key per rung via `mark_align_feasibility` → per-rung `_marked.jpg` + verdict/score, and flag the `best_rung` where the key is present). **Wheel target is always the detected live SEM-box center** — the cursor is moved into the box + the window activated before each scroll; if the box can't be located the ladder is skipped (no scrolling the window center). Sweep order: OUT arm → return to baseline (wheel symmetry; PM-verified) → IN arm; end mag left as-is. No click/recenter (pure wheel + capture). Trigger verdicts: `_ZOOM_PROBE_VERDICTS` in `monitor/cycle.py`. Real wheel requires `SAFE_MODE=0` (else `[DRY-RUN]`, but capture still runs — a good way to confirm the path executes). Saves `<tag>_rcs_zoom_{out,in}{N}.jpg` (+ `_sembox.jpg` crop, + `_marked.jpg` when rematch) + `<tag>_zoom_ladder.json` next to the capture. The PM-button→dropdown (absolute-mag) method is a documented future option (not built).
