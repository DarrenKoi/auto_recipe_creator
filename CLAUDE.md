...
uv run pytest poc/workflow_3/monitor/test_rcs_recovery.py             # 12 (RCS 재실행 중복 가드 + 복구 로그인이 tool 안 여는 계약)
