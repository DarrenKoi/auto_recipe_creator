...
- `matching/mind_rerank.py` — MIND(self-similarity) score-only rerank, RRF-combined with the NCC selection order inside `compute_align_key_score_ensemble` (ported 2026-07-20 from the workflow_2 registration A/B: prod_mind d=+0.042 > prod d=+0.009, SEM +0.057, 67 recipes/334 pts). Never emits new coordinates (picks among existing candidates); all-rejected → NCC selection unchanged. Kill switch `ALIGN_FAIL_MIND_RERANK=0`. Keep constants bit-parity with `poc/workflow_2/registration_lab.py` (the bench measures against this implementation).
