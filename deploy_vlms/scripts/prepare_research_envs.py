"""다중 사이즈 연구용 model env 파일을 생성한다.

사용법:
  python prepare_research_envs.py
  python prepare_research_envs.py ui-venus
...
def selected_variants() -> list[ResearchVariant]:
...
def build_env_text(variant: ResearchVariant, model_root: str) -> str:
    ...
    lines = [
        f"# Research variant: {variant.family} {variant.size}",
...
def main() -> None:
    ...
    log(f"Prepared {len(variants)} research env file(s) under {models_root}")
