...
def build_rcs_select_tool_prompt(
    ...
    lines = [
        ...
        "",
        "Matching rules:",
        "- First try exact match with the requested tool name.",
        "- If exact match does not exist, allow one best partial match.",
        "- If no reliable match exists, return found=false.",
        "- Keep tool-name text exactly as visible.",
        "",
        "Coordinate rules (critical):",
        "- Return one click point on the FIRST LETTER of the matched tool name.",
