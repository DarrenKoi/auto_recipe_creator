...
사용법:
  ...
  2. uv run python poc/work2/login_rcs_ui_venus_mai.py
"""
...
def _save_pipeline_inputs(
    ...
    save_debug_jpeg(zoom_image, zoom_capture_path, log_name=LOG_NAME)
    save_debug_webp(zoom_image, zoom_webp_path, log_name=LOG_NAME)
    return {
...
def _run_mai_ui_refinement(
    ...
    print(f"[INFO] [mai-ui] refine 응답: {response.text!r}")
    print(f"[INFO] [mai-ui] tokens={response.token_usage or {}}")
