...
"왜"의 후보는 여러 개다 — key 자체가 안 유일하다 / 매칭기(matcher)가 약하다 / scale 을 놓쳤다 /
...
아닌 다른 곳에 lock-on** 했다는 직접 증거다. 정답을 알려줘도(at_crosshair) 자유 검색이 더 높은
점수를 딴 곳을 찾아냈다는 뜻 → green mark 가 틀리는 메커니즘 그 자체.
...
- **MI**: 두 이미지를 겹쳐 2D 히스토그램을 만들고 "한쪽을 알면 다른 쪽 불확실성이 얼마나 주나"를
  잰다. 공정 변화로 픽셀값이 통째로 바뀌어도 *구조적 대응*이 남으면 높게 나온다. 그래서 SEM
  align 처럼 **픽셀 동일성을 가정 못 하는**(`project_align_key_matching_constraint`) 문제에 적합.
...
찍기만 해도 정확도 90%가 나온다 — 쓸모없는데 점수만 높다. **bACC 는 S 정답률과 E 정답률을
따로 구해 평균**하므로, 한쪽만 잘 맞혀선 점수가 안 오른다.
...
평소 매칭은 "이미지 어디에 key 가 있나?"를 *모르고* 찾는다(free search). 그래서 틀려도
"key 자체가 약한 건지, 검색이 엉뚱한 데 끌린 건지" 구분이 안 된다.

truth-forced 는 다르다. **S 이미지의 crosshair 위치 = 정답을 안다.** 그 정답 위치 주변 ROI 에서만,
**넓은 scale 대역**(0.5~1.4)으로 chamfer 를 강제로 재본다. 이러면 "검색이 틀린 곳에 갔다"는
변수를 제거하고 **순수하게 "정답 위치에서 매칭이 잘 되나?"** 만 본다.
...
시간 낭비. gt-in-topK 가 바로 이 둘을 **숫자로 가르는 계측**이다.
...
절대 점수("rcp↔msr 유사도가 0.5다")는 함정이다 — matcher 가 약하면 정답에서도 0.5 가 나오니까.
그래서 **상대 비교**를 한다:
...
> 임계와 무관하게 "ratio 가 높아질수록 recall 도 높아지나"의 *추세*를 본다.
...
> 이게 왜 결정적이냐: §5 에서 recall 이 낮다고 나와도, 그게 "rcp 가 낡아서"인지 "matcher 가
> 약해서"인지 모른다. point-biserial 이 **0 에 가까우면** → staleness 핑계를 배제하고 "원인은
