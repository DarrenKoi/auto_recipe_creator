...
이 문서가 바꾸는 것은 "무엇을 먼저 시험하는가" 이지 "무엇이 클릭을 결정하는가" 가 아니다.
...
애초에 후보 목록에 없어서 어떤 재순위로도 고칠 수 없는" 경우**다.
...
후보였다.** 즉 "recall miss 를 고치면 된다"는 명제는 참이지만 불완전하다. 정확히는:

> 되찾은 후보에 **동시에 변별력 있는 점수를 붙일 수 있는 방법**만 유효하다.
...
이라고 묻는다. 답은 세 층위이고, **세 번째가 이 연구의 핵심 발견**이다.

#### (a) 1점 그대로 쓰는 법: heatmap 감독 (가장 안전, 가정 없음)

template crop `T` 와 search 이미지 `F` 를 받아 `F` 위의 dense response map `R` 을 내는 모델을 두고,
...
이 형태의 결정적 장점은 §1.2 의 시소를 구조적으로 회피한다는 것이다. `argmax R` 이 rank-1 이고
...
LoFTR 계열의 학습에는 **dense 대응 라벨**이 필요하다. EfficientLoFTR 은 coarse 단에서 "depth map 과
...
> (`align/matching/engine.py:47`) - 배율을 신뢰했다면 그 탐색이 필요 없다. 즉 **s 는 cond 로 초기값을
> 잡되 pair 별로 정밀화해야 할 값**으로 봐야 한다.
...
90%를 버려도 수십만 pair 가 남는다**. 이것이 "라벨이 많다"의 진짜 가치다: 품질 필터를 사치스럽게 걸 수 있다.
...
- **호스트 RAM 16GB** (H200 140GB x2 이지만 binding constraint 는 GPU 가 아니라 프로세스 수;
  ...
  서비스 프로세스를 추가하는 것은 **모델 하나를 내리는 것과 교환**해야 한다.
- **학습 시 데이터로더의 호스트 RAM**. 수백만 JPEG 을 도는 num_workers 다중 프로세스는 16GB 를 바로
  ...
  또는 학습만 별도 장비. **이건 알고리즘 리스크가 아니라 일정 리스크로 계상해야 한다.**
...
- 약점 1: 자연영상 사전학습의 도메인 갭. 다만 **반증 증거가 있다** - SAR-optical 위성 정합에서
  ...
  ([SEM foundation model, MoE+MAE, 2026](https://arxiv.org/html/2604.05960v1)). 즉 **zero-shot 은 걸어볼
...
- 약점: patch 토큰은 **의미(semantic)** 표현이라 "이 라인과 저 라인은 같은 종류"라고 말한다. 주기 구조의
  phase 를 가르는 데 필요한 것은 정반대의 성질이다. SEM 에서 상관맵이 통째로 평평할 위험이 실재한다.
  이건 1단계 실험이 **싸게 반증해 줄 가설**이다.
...
학습은 정답 위치의 Gaussian label 에 건다. OS2D 는 이것을 **학습에 없던 클래스**로 일반화시킨다:
...
**이 "unseen class 일반화"가 우리에게 결정적인 이유**: production 은 학습에 없던 recipe 를 상대한다.
...
- 출력 계약: `argmax R` -> `best_xy`. **기존 chamfer 후보와 같은 좌표계, 같은 의미**이므로
...
"수백만 쌍"은 tracker 를 처음부터 학습시키는 데 쓰이는 규모와 같은 자릿수다.** 이것이 "학습이
현실적인가"에 대한 정량적 답이다.
...
- 약점 1: tracker 는 **연속 프레임의 작은 변화**를 전제로 설계됐다. 우리 pair 는 수 개월 공정 drift 를
  건널 수 있다. 완화: 학습 쌍을 시간차로 층화(§2.4).
- 약점 2: cross-correlation 은 결국 상관이다. "학습된 chamfer" 로 끝날 위험이 실재한다. 반론은 학습된
  feature 위의 상관은 raw edge 위의 상관과 다르다는 것이지만, 이건 **주장이 아니라 실험이 답할 문제**다.
- OM: 잘 되겠지만 얻을 게 적다. SEM: 이 축의 유일한 시험대.

**추론 비용.** 512x512 search 에서 GPU 10ms 내외, CPU 수백 ms(추정). **CPU 추론이 가능하다는 것이
실무적으로 가장 큰 장점**이다 - 상주 GPU 프로세스를 추가하지 않고 monitor 프로세스 안에서 돌릴 수 있다.
...
정합**이다. 등록 이미지는 수 개월 전, 프레임은 지금이고, 그 사이 공정 drift 로 외관이 변한다.

**학습에서 커버하는 법 - 데이터가 이미 답을 갖고 있다.**

핵심은 **시간차를 학습 쌍의 축으로 명시**하는 것이다. 수백만 장이 있으므로 rcp-msr 쌍만 쓸 이유가 없다.

- `(S_t1, S_t2)` 쌍: 같은 recipe 의 두 성공 프레임. **양쪽 다 crosshair 가 있으므로 대응이 정확히 알려진
  쌍이 공짜로 조합폭발한다.** recipe 당 S 가 n 장이면 쌍은 n(n-1)/2 개.
- 시간차 `|t2 - t1|` 로 층화해 **긴 시간차 쌍을 hard example 로 과대표집**한다. 이것이 drift robustness 를
...
시사한다." 이것은 **판별 정보가 align key 안이 아니라 그 바깥 FOV 에 있다**는 직접 증거다.

여기서 논리를 완성하면:

> 판별 정보가 key 바깥에 있는데, 현재 파이프라인의 점수 함수는 template 크기 window 안에서만 계산된다.
> chamfer/NCC 는 정의상 window 밖을 못 본다. **즉 우리는 정보가 있는 곳을 보지 않는 매처를 쓰고 있다.**

이것이 이 문서가 제시하는 가장 강한 "학습을 해볼 이유"다. 그리고 이건 **반증 가능**하다: 전역 attention
...
**설계상 유일하게 만들어진 전용 마크**다. 우리 문제는 **엔지니어가 제품 패턴 위 임의 영역을 key 로
등록해 그 영역이 유일하지 않을 수 있다**는 것이고, 이 조건을 다룬 공개 연구는 찾지 못했다. 선행 연구는
...
장비가 주기 구조에서 **wrong-phase 위치에 정렬하고도 자기 기준으로 성공**했다면, crosshair 는 decoy 를
...
1. **recipe 내 기하 일관성**: 같은 recipe 의 S crosshair 들은 다이/스테이지 좌표계에서 서로 정합되어야
   ...
   튀는 표본**을 phase-slip 후보로 격리한다. 이건 소손실 기준과 달리 노이즈의 실제 생성 메커니즘을 겨냥한다.
2. **격리 방식은 삭제가 아니라 별도 집합**: phase-slip 후보를 버리면 하드 케이스를 통째로 버린다.
...
**판정: 이건 선택 사항이 아니라 학습 프로그램의 전제 조건이다.** §3 Step 2 착수 전에 1번을 측정해야 한다.
...
나머지 **약 0.456 이 far_wrong** 이다. 즉 그 arm 에서 SEM 오류의 압도적 다수는 **격자 주기의 정수배
phase slip 이 아니라 전혀 엉뚱한 곳**이었다. 이것이 사실이라면 "SEM 은 주기성 때문에 정보가 없다"는
...
**왜 recall@8 만 보지 않는가.** 브리프의 예시는 recall@8 만 재는 것이었지만, ADR 0006 이 정확히 그 함정을
...
- **3a. §1.3(c) dense 승격 + EfficientLoFTR fine-tune.** 선행 조건: 배율 가정 3개(§1.3c) 검증.
  전역 attention 이 §2.5 의 "key 바깥 정보"를 실제로 쓰는지 보는 직접 실험.
- **3b. 도메인 SSL 사전학습 후 Step 2 모델 초기화.** E 프레임(라벨 없음)을 여기서 쓴다. MAE 계열이
  ...
  7월의 P3-I 가 여기로 온다. 단 목적이 바뀌었다 - "라벨이 없어서"가 아니라 "E 프레임을 쓰려고".
...
2. **장비(eqp) 분리 보조 표**: consensus pool 은 eqp 무관이지만 캡처 특성은 장비별로 다르다.
   최소한 별도 표로 보고한다.
3. **시간 분할**: test 를 train 보다 **미래**로 둔다. 실제 배포는 항상 미래 프레임을 상대한다.
...
즉 **0.876 / 0.764 는 성공 프레임 위에서 측정된 숫자**다. production 은 align **fail** 프레임 위에서
돈다. E 프레임에는 crosshair 가 없어 GT 가 없으므로 채점 자체가 불가능하고, 이건 기존 벤치의
한계이지 이 문서가 만든 문제가 아니다. 그러나 학습을 도입하면 **모델이 S 분포에 최적화되고 E 분포에서
평가되지 않는** 상태가 되므로 위험이 커진다.

**요구사항**:
- 최소한 E 프레임에서 **점수 분포와 abstention 율**을 보고한다(정확도는 못 내도 분포는 낸다).
- S 와 E 의 응답맵 통계(최대값, 2등 비, 엔트로피)가 유의하게 다르면 그것 자체가 배포 위험 신호다.
- 진짜 종단 지표는 오프라인 rank1 이 아니라 **운영 루프의 corrected vs escalated 비율**이다.
...
- 사전학습 가중치는 오피스로 반입(단방향). 라이선스는 도입 전 확인 필요 -
  **특히 DINOv3 는 Meta 자체 라이선스**이고 LoFTR/LightGlue 계열과 조건이 다르다(확인 필요).
...
만들지 못한다**. 이 경우 Bayes-optimal 출력은 점이 아니라 집합이고, 올바른 운영 행동은
...
나머지 약 0.456 은 far_wrong 계열이다. 격자 주기의 정수배 오류는 SEM 오류의 **약 10분의 1**이고
...
존재한다는 뜻**이고, 실제 다이에는 그런 것이 있다: 어레이 경계, 다이 경계, dummy fill 경계, 스캔/차징
그라디언트. 유한한 어레이는 무한 격자가 아니다.

**증거 3 (구조적).** 현재 점수 함수는 template 크기 window 안에서만 계산된다. chamfer 도 NCC 도 window
밖을 정의상 못 본다. **즉 증거 2 가 가리키는 정보를 현재 매처는 원리적으로 사용하지 않는다.**
...
3. **라벨 노이즈가 하드 케이스와 상관된다**(§2.6). 표준 노이즈 대응 도구가 역작동한다.
   대응이 안 되면 "오프라인 지표는 좋은데 현장에서 decoy 로 간다"가 될 수 있고, 이건 오프라인 평가로는
   안 잡힌다.
...
6. **재등록은 어느 쪽이든 필요하다.** 학습이 성공해도 chronic-ambiguous key 는 재등록이 정답이다.
   이 연구는 재등록 워크스트림의 대안이 아니라 그것과 병행하는 축이다.
