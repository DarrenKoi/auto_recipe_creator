...
> Mac 에서는 합성 self-test 만 가능하고, 실판정은 오피스 실데이터로만 한다.
...
| rcp 이미지에 **흰 box** 있음 | `box` template 셀(=NEW)의 필수 입력 | box 셀이 비어 verdict 가 "비교 불가" — OLD baseline 만 돈다 |
...
- `[WARNING] golden 데이터 없음 ... → 합성 self-test` 가 뜨면 **데이터를 못 읽은 것**이다.
  → 레이아웃/경로(특히 `align_img_from_rcp` / `align_img_from_msr` 디렉터리명)를 다시 확인한다.
...
- **`gt_in_topk_rate`** = proposer recall. NEW 가 여기서만 높고 rank1 은 안 오르면 → 정답이 후보엔
  들지만 1등은 아니라는 뜻 → **reranker 로 메울 수 있는 갭**(`topk_not_rank1_rate` 가 그 크기).
...
Mac 에서 합성 self-test 로 파이프라인이 도는 것은 이미 확인됐다(코드 무결성 보장).
따라서 오피스에서 실패하면 원인은 **코드가 아니라 데이터**로 좁혀진다 — 본 런북의 §1 체크리스트부터 확인한다.
