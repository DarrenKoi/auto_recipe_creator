...
CV 는 edge 가 정확히 어디 있는지 *측정*하는 데 강합니다. 각자 잘하는 일을 시키는 것입니다.
...
wafer 영상, 다른 쪽은 밝은 UI 프레임입니다. 그 경계를 가로지르면 숫자가 점프합니다.
...
한 픽셀에서의 "변화량"을 **gradient** 라고 합니다. 이미지판 *기울기(slope, 미분)*입니다: 어떤
...
> **왜 교차될까요?** *세로* 벽은 왼쪽이 밝고 오른쪽이 어두운 픽셀 열입니다 — 변화는 **가로로**
> 가로지를 때 일어납니다. 그래서 세로 edge 는 가로 변화(`grad_x`)로 검출합니다. 가로 edge 는
> 반대입니다. 입문자가 꼭 헷갈리는 부분이니 두 번 읽으시길 권합니다.
...
이것이 *VLM box 가 "대략적"인 이유*입니다: 1920px 폭 스크린샷에서 1 격자 단위 ≈ 2px 이고, 모델은
눈대중한 edge 로 반올림하므로 보통 **5–15px 어긋납니다**. "어디를 볼지"에는 충분하지만 클릭
기준으로는 부족합니다. 바로 그 간극을 Pass 2 가 메웁니다.
...
*폭 전체*에 걸쳐 `grad_y` 가 강합니다. box 내부의 노이즈/질감은 흩어진 단일 픽셀에서만 강합니다.
...
snap 된 bottom 이 snap 된 top 보다 위로 갈 수 있습니다. 그것은 말이 안 되는(뒤집힌) 사각형입니다.
이를 방지합니다:
...
- **Laplacian** 은 2차 미분 필터입니다 — 모든 방향의 *미세 디테일과 edge*에 반응합니다. 선명한
  이미지는 또렷한 edge 로 가득 차서 Laplacian 에 큰 값이 많습니다. 흐린 이미지는 뭉개진 완만한
...
cyan box 가 실제 경계를 딱 감싸고 magenta 가 그 주변에 헐겁게 있으면, 전체 파이프라인이 설계대로
동작하는 것입니다.
...
이제 원리를 알았으니, 현재 로직이 취약한 지점들을 정리합니다:
