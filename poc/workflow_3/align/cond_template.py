...
def centered_area_crop_bbox(gray, area_ratio=CENTER_AREA_RATIO):
    """이미지 중심 기준 *면적 비율* crop bbox 를 (x, y, w, h) 로 반환한다."""
    h, w = gray.shape[:2]
...
def centered_area_crop(gray, area_ratio=CENTER_AREA_RATIO):
    ...
    중심이라는 사전지식을 살려 매칭을 중심부에 집중시킨다.
    """
    x, y, cw, ch = centered_area_crop_bbox(gray, area_ratio)
