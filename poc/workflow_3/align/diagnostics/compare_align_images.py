...
입력: `align.assets.resolve_assets_auto()` 가 해석한 recipe 의
recipe_sem(from_rcp/IMAP0002) 와 current_sem(from_msr 최신 E*).
(없으면 합성 self-test 로 파이프라인만 점검).

실행:
    uv run python poc/workflow_3/align/diagnostics/compare_align_images.py
"""
...
# ====================================================================
# recipe 폴더 선택은 align.assets 가 담당(환경변수 override 또는 최신 자동).
...
def _make_selftest_pair() -> tuple[np.ndarray, np.ndarray]:
    ...
    from poc.workflow_3.align.matching.test_engine import (  # 재사용: 합성 패턴/배경 생성기.
        embed_pattern,
