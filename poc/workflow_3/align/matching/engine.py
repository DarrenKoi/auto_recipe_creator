...
# ------------------------------------------------------------------
# matching.ensemble 은 *lazy* 로 로드한다 — 순환 참조 회피 + import 순서 무관.
#
# matching.ensemble 이 이 모듈(engine)에서 여러 심볼을 임포트하므로 상호 의존이다.
# 모듈 말단 import 로 두면 engine 이 먼저 로드될 때만 동작하고, matching.ensemble 이
# 먼저 import 되면(예: test_ensemble 단독 실행) 부분 초기화 순환 import 로 깨진다.
...
# ------------------------------------------------------------------
compute_ensemble_candidates = None  # lazy: 최초 호출 시 matching.ensemble 에서 바인딩.
