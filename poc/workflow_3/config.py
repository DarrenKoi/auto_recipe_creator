...
class Workflow3Settings(WorkflowSettings):
    ...
    # 녹화 보관 run 수 - 사이클 끝에 최신 N 개 recording/ 폴더만 남긴다. 0 = 삭제 안 함.
    # 2026-09-17 사용자 결정(30). _manual 세션과 캡처/Episode JSON 은 대상이 아니다.
    recording_keep_runs: int = 30
    ...
    # vlm/ui_venus_mai_locator.py 의 DEFAULT_* 상수). 형식은 "coarse>fine".
    # 2026-09-05 현재 grounding 후보는 mai-ui 하나뿐이라 실질적 대안이 없다
    # (ui-venus 는 2026-09-03 가중치 삭제, mai-ui-2b 는 2026-09-05 폐기).
...
def load_workflow3_settings() -> Workflow3Settings:
    ...
    return Workflow3Settings(
        **base.to_snapshot(),
        ...
        recording_keep_runs=env_int("ALIGN_FAIL_RECORDING_KEEP_RUNS", 30),
