...
> 절차이고, 기본 답은 **아니오** 다. RAM 이 진짜 병목이면 호스트를 32GB 로 올리는 것이
> 정답이지 grounding 정확도를 거래하는 게 아니다.
> 그래도 재보고 싶다면(예: 32GB 증설이 불가능해졌다) 아래를 따른다.
...
잃는 게 있으면 하지 않는 편이 낫다. RAM 이 정말 문제면 호스트를 32GB 로 올리는 것이
정답이고, 이 A/B 는 그 대안이 아니다.
