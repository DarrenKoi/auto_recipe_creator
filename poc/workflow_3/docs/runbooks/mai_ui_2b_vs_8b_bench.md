...
> **OBSOLETE (2026-09-05).** mai-ui-2b 는 폐기됐다 - 서버 route stub / registry /
> `config/models/mai-ui-2b.env` 와 workflow_3 클라이언트 등록이 모두 제거돼서 이 절차는
> 그대로는 실행되지 않는다. 남겨 두는 이유는 절차가 아니라 **판단 근거**다: 왜 8B 를
>유지했는지, A/B 의 상방이 왜 없는지가 아래에 있다. 다시 하려면 배선을 되살려야 한다
> (git 이력이 복원 경로).
