...
`ASSIST_NEWEST_ROW_AT = "bottom"` (모듈 상수, 기본값). 최신 행이 위에 쌓이는 것으로 확인되면
`"top"` 으로 바꾼다. 오피스 1회 실행의 디버그 오버레이로 즉시 확인된다.
...
- 측정점이 6개 미만인 레시피: streak 6 에 도달하지 못해 감지되지 않는다. 안전한 실패이므로
  이번 범위에서 다루지 않는다.
