...
입력 고르기 (셋 중 하나):
  1) DEMO_COMBINED_INPUT_DIRS 에 recording 폴더를 콤마로 나열 (적은 순서 그대로)
  2) DEMO_COMBINED_ROOT 아래의 모든 recording 폴더를 자동 수집 (보통
     `.../captured_img_from_rcs` 를 가리킨다)
  3) 아무것도 안 주면 ALIGN_IMAGES_DIR 전체를 훑어 recipe 별로 묶고 **가장 최근에
...
사용법:
  ...
  DEMO_COMBINED_INPUT_DIRS="<...>/260824_101530/recording,<...>/260824_104512/recording" \
    uv run python poc/workflow_3/monitor/make_demo_video_combined.py

조합 env (이 스크립트 전용):
  DEMO_COMBINED_INPUT_DIRS  recording 폴더 목록 (콤마 구분; 이 순서를 그대로 씀)
  DEMO_COMBINED_ROOT        이 폴더 아래의 recording 을 전부 모은다
  DEMO_COMBINED_OUTPUT      출력 파일 (기본 <공통부모>/demo_combined.mp4)
  DEMO_COMBINED_LABELS      회차 라벨 "Baseline,After tuning" (기본 1st/2nd/3rd Trial)
  DEMO_COMBINED_TITLE_SEC   회차 타이틀 카드 노출 시간 (기본 2.5, 0=카드 없음)
  DEMO_COMBINED_SEGMENTS    모든 회차에 적용할 남길 구간 "0-30,120-"
  DEMO_COMBINED_SEGMENTS_2  2회차에만 적용 (1-based; 개별 지정이 공용보다 우선)
  DEMO_COMBINED_PREVIEW     회차별 타임라인 미리보기 출력 (기본 0)

렌더 env 는 `make_demo_video` 와 **같은 이름을 그대로 쓴다** (두 스크립트에서 같은
뜻이어야 하므로 새 이름을 만들지 않는다):
  DEMO_VIDEO_SPEED / FPS / MAX_HOLD_SEC / TAIL_HOLD_SEC / MAX_WIDTH / OVERLAY / PRELUDE
