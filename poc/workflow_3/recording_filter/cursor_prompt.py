...
def cursor_system_prompt() -> str:
    ...
    return (
        ...
        "\n"
        "CRITICAL - this application draws a static OPEN-PALM hand icon (all fingers spread, no single "
        "extended finger) as a toolbar control, positioned between the 'Full Size' button and the live "
        "SEM image area. It is part of the window chrome: it is in EVERY screenshot at the SAME place "
        "and it never moves. It is NOT the mouse cursor. Never report it. If the only hand-like shape "
        "you can find is that icon beside the live SEM image, answer cursor_visible=false.\n"
        "Tell them apart by shape (one extended finger = cursor, open palm = icon) and by context "
        "(the cursor can be anywhere on screen; the palm icon is always in that one spot).\n"
        "\n"
