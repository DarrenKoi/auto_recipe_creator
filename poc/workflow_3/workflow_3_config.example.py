...
# 결과-후-알림의 침묵 상한(초). 사이클이 이 시간을 넘기면 "자동 보정 진행 중" 을 1회
# 고지한다. 0=끄기(결과 나올 때까지 완전 침묵). None(기본 90).
...
# ============================================================================
# [7.2] 점유 tool 화면 공유 요청 (2026-08-18)
# ----------------------------------------------------------------------------
# 다른 엔지니어가 tool 을 쓰고 있으면 'Select' 팝업에서 "화면 공유"를 요청하고,
# 승낙되면 관전 세션으로 들어가 그 엔지니어의 수동 작업을 녹화한다.
SHARE_REQUEST = None         # 공유 요청 발송. 1/0/None(기본 on).
# 클릭 전 라벨 OCR 확인 정책. "strict"(기본)=확인된 것만 클릭 / "lenient"=못 읽어도
# 클릭 / "off"=확인 생략. 어느 값이든 terminate/control/cancel 이 읽히면 클릭하지 않는다.
...
ENGINEER_DONE_DETECT = None  # 완료 신호(창 닫힘/분자 증가/커서 2분 정지)로 watch 조기 종료.
                            # 1/0/None(기본 **on**, 2026-08-19). 끄면 항상 cap(5분)까지 간다.
                            # 커서 정지 상한은 env ALIGN_FAIL_ENGINEER_DONE_IDLE_SEC=120 (0=끔).
...
# 형식은 "coarse>fine". 상시 기본을 바꾸려면 코드 상수를 고친다.
# (ui-venus 2026-09-03 가중치 삭제, mai-ui-2b 2026-09-05 폐기 - 지금 후보는 mai-ui 뿐이다.)
#
